...
import { COMMISSION_POST_MIDDLEWARE } from "./Prisma-Middleware/CommissionMiddleware";
/**
 * Prisma client
...
prisma.$use(async (params, next) => {
  try {
    ...
    if (params.model === "Commission" && result) {
      const processedResult = await COMMISSION_POST_MIDDLEWARE(params, result);
      if (processedResult) {
        result = processedResult;
      }
    }
